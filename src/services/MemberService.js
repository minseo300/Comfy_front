...
export async function login(idToken){
    ...
    const response=await axios.get(`http://10.108.24.189/login/google/${idToken}`).then((response)=>{
...
export async function logout(){
    ...
    const response=axios.delete(`http://10.108.24.189/logout/${localStorage.getItem('memberId')}`);
