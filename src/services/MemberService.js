...
export async function login(idToken){
    ...
    const response=await axios.get(`http://172.16.1.245/login/google/${idToken}`).then((response)=>{
...
export async function logout(){
    ...
    const response=axios.delete(`http://172.16.1.245/logout/${localStorage.getItem('memberId')}`);
