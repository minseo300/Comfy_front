...
export async function getPosts(){
    ...
    const response=await axios.get(`http://spring-boot-docker-service:8080/community/${memberId}`);
...
export async function getMyPagePosts(){
    ...
    const response=await axios.get(`http://spring-boot-docker-service:8080/myPage/${memberId}`,{headers:config});
...
export async function deleteMyPost(postId){
    ...
    const response=await axios.delete(`http://spring-boot-docker-service:8080/post/${postId}/${memberId}`,{headers:config});
...
export async function addBookmark(postId){
    ...
    const response=await axios.post('http://spring-boot-docker-service:8080/bookmark',data,{headers:config});
...
export async function deleteBookmark(postId){
    ...
    const response=await axios.delete(`http://spring-boot-docker-service:8080/bookmark/${postId}/${memberId}`,{headers:config},{});
...
export async function getPostInfo(postId){
    ...
    const response=await axios.get(`http://spring-boot-docker-service:8080/post/${postId}/${memberId}`,{headers:config});
...
export async function createPost(title,content,surveyId){
    const response=await axios.post(
        "http://spring-boot-docker-service:8080/posting",{
            title:title,
            contents:content,
            memberId:localStorage.getItem('memberId'),
            surveyId:surveyId
...
export async function getSearchedPosts(word){
    const response = await axios.get(
        "http://spring-boot-docker-service:8080/search?word=" + word,{
          params:{
              title:word
          }
        }
