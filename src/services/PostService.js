...
export async function getPosts(){
    ...
    const response=await axios.get(`http://172.16.1.245/community/${memberId}`);
...
export async function getMyPagePosts(){
    ...
    const response=await axios.get(`http://172.16.1.245/myPage/${memberId}`,{headers:config});
...
export async function deleteMyPost(postId){
    ...
    const response=await axios.delete(`http://172.16.1.245/post/${postId}/${memberId}`,{headers:config});
...
export async function addBookmark(postId){
    ...
    const response=await axios.post('http://172.16.1.245/bookmark',data,{headers:config});
...
export async function deleteBookmark(postId){
    ...
    const response=await axios.delete(`http://172.16.1.245/bookmark/${postId}/${memberId}`,{headers:config},{});
...
export async function getPostInfo(postId){
    ...
    const response=await axios.get(`http://172.16.1.245/post/${postId}/${memberId}`,{headers:config});
...
export async function getSearchedPosts(word){
    const response = await axios.get(
        "http://172.16.1.245/search?word=" + word,{
          params:{
              title:word
          }
        }
